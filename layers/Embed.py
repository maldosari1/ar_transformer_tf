...
class TokenEmbedding(nn.Module):
    def __init__(self, c_in, d_model, model):
        super(TokenEmbedding, self).__init__()
        self.model = model

        if(self.model == 'Transformer_TF'):
...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1, model='Transformer', default_pe=True, glom=True):
        super(DataEmbedding, self).__init__()
        self.model = model

        self.value_embedding = TokenEmbedding(c_in=c_in, d_model=d_model, model=model)
        self.position_embedding = PositionalEmbedding(d_model=d_model, default_pe=default_pe)
